...
export default function CompCarrousel() {
  ...
  useEffect(() => {
    async function loadNovidades() {
      const response = await api.get("/novelties");
      setNovidades(response.data);
    }
    loadNovidades();
  }, []);

  const filterCategory = [...new Set(novidades.map((item) => item.category))];

  return (
    <div className="2xl:mx-auto w-full flex flex-col justify-center items-center">
      <div className="w-full">
        {filterCategory.map((category) => (
          <div key={category._id}>
            <div className="text-3xl mt-10 max-w-[1200px] mx-auto font-bold first-letter:uppercase">
            ...
            >
              <div className="w-full relative flex items-center justify-center">
                <ButtonBack
                  ...
                  className="absolute z-30 left-6 top-[12rem] ml-8 bg-dark p-1 focus:outline-none focus:bg-dark focus:ring-2 focus:ring-offset-2 focus:ring-[#121212] cursor-pointer"
                  id="prev"
                >
                  <svg
                    width={8}
                          ...
                          <Slide index={index}>
                            <div class="flex flex-col items-center justify-between py-4 w-full h-[35rem] bg-gradient-to-b from-primary from-65% via-secondary via-100% to-dark rounded-lg">
                              ...
                              <div className="w-full">
                                ...
                                <p className="font-semibold text-[13.5px] px-2">
                                  {item.price >= 1000
                                    ...
                                    : `10x de R$ ${(item.price / 10).toFixed(
                                        2
                                      )}`}
                                </p>
                              </div>
                              <div className="flex justify-center w-full">
                                <a
                                  href={`/detalhes/${item.category}/${item._id}`}
                                >
                ...
                <ButtonNext
                  ...
                  className="absolute z-30 right-6 top-[12rem] mr-8 bg-dark p-1 focus:outline-none focus:bg-dark focus:ring-2 focus:ring-offset-2 focus:ring-[#121212] cursor-pointer"
                  id="next"
                >
                  <svg
                    width={8}
            ...
            >
              <div className="w-full relative flex items-center justify-center">
                <ButtonBack
                  role="button"
                  aria-label="slide backward"
                  className="absolute z-30 left-[2rem] top-[13rem] ml-8 bg-dark p-1 focus:outline-none focus:bg-dark focus:ring-2 focus:ring-offset-2 focus:ring-[#121212] cursor-pointer"
                  id="prev"
                >
                  <svg
                    width={14}
                ...
                <ButtonNext
                  role="button"
                  aria-label="slide forward"
                  className="absolute z-30 right-[2rem] top-[13rem] mr-8 bg-dark p-1 focus:outline-none focus:bg-dark focus:ring-2 focus:ring-offset-2 focus:ring-[#121212] cursor-pointer"
                  id="next"
                >
                  <svg
                    width={14}
